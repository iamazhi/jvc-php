...
class router
{
    ...
    {
        $modulePath = $this->getModulePath($this->moduleName, 'control');

        $this->controlFile = $modulePath . DS . 'ext' . DS . $this->moduleName . '.php';
        if(is_file($this->controlFile)) return true;

        $this->controlFile = $modulePath . DS . $this->moduleName . '.php';
     ...
     */
    public function getModuleExtPath($moduleName, $type)
    {
        $extPath = $this->getModulePath($moduleName, $type) . 'ext' . DS;
        return $extPath;
    }
    ...
    {
        $extPath = $this->getModuleExtPath($this->moduleName, 'control');

        $this->extActionFile = $extPath . $this->moduleName . '.php';
     ...
     * 
     * If the module is system, search in $configRoot, else in $modulePath.
     *
    ...
    public function loadConfig($moduleName, $exitIfNone = true)
    {
        ...
        {
            $mainConfigFile = $this->getModulePath($moduleName, 'config') . $moduleName . '.php';
        ...
        if(empty($configFiles) and $exitIfNone)  self::triggerError("config file $mainConfigFile not found", __FILE__, __LINE__, true);
        ...
        if($moduleName != 'system' and isset($config->system->$moduleName))
        {
    ...
    {
        $langPath = $this->getModulePath($moduleName, 'lang');
        $mainLangFile = $langPath . $moduleName . '.php';

        /* get ext lang files. */
        $extLangPath = $this->getModuleExtPath($moduleName, 'lang');
        $extLangFile = $extLangPath . $moduleName . '.php';

        /* Set the files to includ. */
        if(!is_file($mainLangFile))
        {
            if(empty($extLangFile)) return false;  // also no extension file.
            $langFiles = $extLangFile;
        }
        else
        {
            $langFiles = array_merge(array($mainLangFile), array($extLangFile));
        }
